...
| `steering-git-workflow` | Git workflow policy: branch discipline, ship-path choice (merge vs PR), merge flags, draft-first PRs, Beads body linkage, and pre-push verification. Ships a dual-runtime PreToolUse blocker for non-draft or unlinked PR creation. Replaces the retired commit-push-* skills and git nudge hooks. |
...
| `steering-multi-agent` | Multi-agent coexistence steering: tolerate concurrent agents and human authors in the same repo or worktree, fix straightforward pre-existing issues in passing, never revert or report benign upstream changes, and escape interference (including branch switches underneath you) by moving to a Worktrunk checkout instead of fighting back. Always-on steering. Opt-in. |
| `steering-pragmatic` | Opinionated pragmatic working style: prioritize accuracy, calibration, and decision usefulness over agreeableness. Analytical and direct by default; distinguish facts from assumptions; avoid hype and sycophancy. Includes code economy defaults (prefer deletion, stdlib, and minimal solutions over new abstractions) and comment discipline. Always-on steering as main-session instructions, plus a SubagentStart hook that injects the same discipline into every subagent in MUST register. Opt-in. |
