...
| `session-review` | Audit the ending session for corrections, lessons, unresolved TODOs, and follow-up work, then recommend handoffs to `handover`, `audit-steering`, `optimize-steering`, or `write-agentic`. Use when the user says wrap up, review this session, what did we learn, or before a handover. |
