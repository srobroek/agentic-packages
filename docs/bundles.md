...
| `core-global` | Recommended global (user-scope) baseline bundle | `project-setup`, `brownfield-project`, `project-hygiene`, `agent-management`, `find-tools`, `session-review`, `catchup`, `codebase-memory`, `debate`, `eli5`, `handover`, `write-a-skill`, `whats-new`, `chezmoi-editor`, `agent-coder`, `agent-pr-reviewer`, `agent-adversarial-challenger`, `agent-external-repo-worker`, `grill-me`^ |
