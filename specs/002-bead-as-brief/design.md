...
Contracts bind to claims and are enforced by hooks. Ephemeral coordination
rides wisps; durable evidence stays on beads. The orchestrator spawns and
observes; it never relays content and never claims.
...
6. **The orchestrator routes that work happens, never what is said.** It
   ...
   bead-to-bead between actors. It never reads target-scope content or
   computes a node's requested result to verify its own workers.
...
```jsonc
{
  ...
  "execution_task_kind": "<code|docs|research|review|operations>",
  ...
  // Orchestrator before activation
  "actor":           "<stable claim identity>",
  "branch":          "<working branch name>",     // Worktrunk-backed actor
  "worktree":        "<abs worktree path>",
  "lease_token":     "<Worktrunk lease>",
  "runtime_context": "<spawned runtime id>",
...
- Both shepherd actors follow the same landing invariant: claim one merge
  bead, probe or land, release. The merge slot and sheepdog serialize them.
...
merges become PRs against the integration base, landed by the bundled
shepherd. The standalone pr-shepherd provides cross-run recovery.
...
- Predicate vocabulary stays tiny: metadata-key-exists, label-matches,
  resource-comment-verb, linked-comment-verb, state-in-set, and
  wisp-open/closed. Link hydration is evaluator-owned; other cross-bead checks
  are scripts, not rules.
...
| Hook | Attachment | Job |
|---|---|---|
| Run activation | UserPromptSubmit, matcher-less, both runtimes | Create the run marker before the lead's first tool call; preserve a prior run id on restart |
...
or raise a human gate. It never repairs the same runtime with out-of-contract
messages or accepts evidence from that bounced attempt. `review_round` = 3 is
the same pattern: the verdict lands, but the orchestrator arbitrates instead
of spawning round 4.
...
- Worktree creation stamps a `[wisp:recovery] wipe-worktree <abs-path>` wisp.
- `bd dep add <wipe-wisp> <merge-bead>` blocks that wisp on the merge bead.
- Closing the merge bead unblocks the wisp after merge or dismissal.
- The next shepherd patrol runs `worktree-sweep.sh`, which uses `wt list` and
  `wt remove`, then closes the wisp.
- Raw `git worktree` lifecycle commands and harness-created worktrees remain
  prohibited.
...
1. **Repository-scoped build target.**
   - The global Worktrunk `pre-start` hook materializes an ignored,
     marker-owned `.cargo/config.toml` in each Rust checkout.
   - Worktrunk renders `{{ primary_worktree_path }}`. The hook persists
     `<primary_worktree_path>/target` as the absolute `build.target-dir`.
   - The hook refuses to overwrite an existing Cargo config. That repository
     must configure the target in its project hook or tracked config.
   - A hook-only `CARGO_TARGET_DIR` export is insufficient because later build
     shells do not inherit it.
   - Project hooks own analogous output settings when the tool has no safe
...
| `supersedes` | Replacement nodes (old node auto-closes with forward pointer) |
...
There are no direct merges and no integration-gatekeeper. Node branches PR
into the run's integration base; the integration branch PRs into main. The
bundled `shepherd` performs this work while the run is live. The standalone
`pr-shepherd` performs repository-global draining and recovery outside a live
run. Both use the same landing safeguards for stacked bases (`pr-base` vs
`landing-base`), draft handling, head validation, bounce-back, and merge-slot
serialization.
...
- One live shepherd per repo. The in-run actor is
  `shepherd-<run>-<repo>`; the global actor is `pr-shepherd-<repo>`.
  Both contend on the same sheepdog and merge slot, so they cannot overlap.
...
| In-run shepherd | `shepherd-<run>-<repo>` | `shepherd-orc-a1b-agentic-packages` |
| Global shepherd | `pr-shepherd-<repo>` | `pr-shepherd-agentic-packages` |
...
- [ ] Claim-holder allocation prompts use only canonical WAIT; activation
      messages are `CLAIM {bead-or-wisp-id}` or `CLAIM queue:{q}`; no ASSIGN
      field survives in any prompt template.
...
- [ ] `workflow-coder`, `workflow-worker`, `workflow-pull-worker`,
      ...
      bundled shepherd and standalone pr-shepherd remain distinct while
      sharing fix-bead, content-read-only, sheepdog, and landing safeguards.
