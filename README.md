...
- **37 skills** -- reusable workflows, each its own package (catchup, code-review, research, verify, ...)
...
Consequence: installing a **steering** package (`steering-*`, `language-steering-*`) — or the steering *content* inside a few mixed packages — via native `/plugin install` / `codex plugin add` is a **silent no-op**: it "installs" but contributes nothing (no error, no rules). Steering is delivered only by the APM CLI, which maps `.apm/instructions` → `.claude/rules/` (always-on):
