...
def test_accepts_matching_latest_tag(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(
        MODULE, "MANIFEST", _write_json(tmp_path, "manifest.json", {".": "1.2.3"})
    )
...
def test_allows_first_release_sentinel_without_tags(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(
        ...
        "CONFIG",
        _write_json(
            tmp_path, "config.json", {"packages": {"new": {"component": "new"}}}
        ),
    )
    monkeypatch.setattr(
...
def test_rejects_first_release_sentinel_after_publication(monkeypatch, tmp_path):
    monkeypatch.setattr(MODULE, "_current_branch", lambda: "fix/example")
    monkeypatch.setattr(
        MODULE, "_component_versions", lambda component: {"1.0.0": (1, 0, 0)}
    )
    monkeypatch.setattr(
        MODULE,
        "CONFIG",
        _write_json(
            tmp_path, "config.json", {"packages": {"pkg": {"component": "pkg"}}}
        ),
    )
    monkeypatch.setattr(
...
def test_skips_release_please_branch(monkeypatch):
    monkeypatch.setattr(
        MODULE, "_current_branch", lambda: "release-please--branches--main"
    )
