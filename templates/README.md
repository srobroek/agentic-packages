...
| `agent` | One subagent definition the main thread delegates to. | `apm.yml` (`type: hybrid`) + `.apm/agents/<name>.agent.md` | There is no `type: agent` in this repo -- agent packages use `type: hybrid`. Keep `x-agentic` (the cross-tool source of truth for Codex/Claude overrides). |
