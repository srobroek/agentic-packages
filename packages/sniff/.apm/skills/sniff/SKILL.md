...
1. **Resolve target & detect stack.** If user did not name a target, STOP and ask.
   ...
   present in the target and map each to `references/languages/index.md`.
2. **Probe & propose the full tool set (mandatory blocking checkpoint, interactive runs).**
   Run `scripts/install-tools.sh --probe`, enumerate every viable tool per detected
...
7. **Report & (optional) apply.** Emit the prioritized plan via `references/report-template.md`.
