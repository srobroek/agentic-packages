...
- **Invocation:** the shipped ruleset path is **relative to the skill dir, not
  the target repo** — and Step 3 runs tools with cwd = the target (or a worktree).
  So ALWAYS pass it as an absolute path, or semgrep silently matches nothing (the
  worst failure: looks clean). First capture the skill dir (the directory
  containing this skill's `SKILL.md`) as `SNIFF_SKILL_DIR`, then:
  ...
  The same rule applies to any bundled-asset path: absolutize against
  `$SNIFF_SKILL_DIR` before use, because cwd is the target, not the skill.
