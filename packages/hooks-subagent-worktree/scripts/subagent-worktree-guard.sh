...
if [[ -n "$stale" ]]; then
  ...
  stale_notice="Stale worktree notice: this repo has linked agent worktrees: ${stale_list}— review whether each is still in use. For every finished one, first CONFIRM IT IS CLEAN: git -C <path> status --porcelain prints nothing and the branch is merged or harvested. Never discard uncommitted work to force a removal — commit, stash, or escalate instead. Once confirmed clean, delete build artifacts (rm -rf <path>/target and similar gitignored output) then remove: git worktree remove <path>; git worktree prune. Dead worktrees accumulate compiled output and fill the disk."
fi
