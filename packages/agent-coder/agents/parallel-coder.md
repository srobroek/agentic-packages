...
MUST Hand-roll only what stays trivially small AND maintainable: before writing it, price the full implementation — parsing, edge cases, escaping, tests, and who debugs it later. Reimplementing a solved category (parsers, template engines, date/time handling, retries/rate-limiting, caching, crypto) is NOT economy — take the maintained library. Tripwire: if your stdlib plan needs a parser, a grammar, or more than two of {loops, conditionals, inheritance, escaping}, you are re-implementing a library. A stated preference for fewer dependencies does not outrank the stated functional requirements.
