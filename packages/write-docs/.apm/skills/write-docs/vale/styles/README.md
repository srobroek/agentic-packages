...
The rules ported from the retired `slop-lint.py`, plus `NoUnicodeDash`.
...
a sync, where each disable and demotion carries the measured reason that earned
it. Read those comments before re-enabling anything.
