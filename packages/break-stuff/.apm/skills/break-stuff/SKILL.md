...
## STOP: two questions before you touch the target

Answer BOTH before detecting the stack, running a scanner, or spawning a
`gremlin`. Neither question is skippable in an interactive run.
...
1. **Which target?** Ask in two steps when the user named none. Offer every time:
   ...
   a scope error.
2. **Which tools, and what fuzz budget?** Run
...
exception: use the target it was given or the whole repo, the budget defaults from
`references/fuzzing.md`, and the installed tools, then record every gap.
...
- **audit-only**: steps 1 to 9, with patching disabled even on approval.
