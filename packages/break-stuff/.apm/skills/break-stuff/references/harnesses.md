...
harness. `fuzzer`'s job here is the vectors file:
...
Cover every wrapper form from `surfaces/shell.md`, plus one benign vector per
guarded pattern to catch over-blocking.
