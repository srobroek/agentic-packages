...
Use every mature ruleset available: the shipped rules under `semgrep-rules/`,
semgrep's registry packs, bandit's and gosec's built-ins, clippy's lint groups.
These are well-tested detectors and rewriting them wastes the campaign's time.
...
| Shipped rules | `semgrep-rules/` in this package | a floor for a repo just met, so a campaign is never empty-handed |
