...
1. **cwd is the target repo root**, never a subdirectory, and never the worktree
   ...
   explicit paths.
2. **Shipped assets are absolute.** Reference `semgrep-rules/` and `corpora/` by
   absolute path under `$BREAK_SKILL_DIR`, since cwd is the target and a
   skill-relative path silently matches nothing.
...
| Asset | Covers | Invoked from |
|---|---|---|
| `semgrep-rules/shell-injection.yml` | `eval` on interpolated data, `curl` piped to a shell, unsafe temp files, missing `set -euo pipefail` | `surfaces/shell.md` |
| `semgrep-rules/taint-sinks.yml` | injection sinks, unsafe deserialization, path traversal, weak crypto | `surfaces/code.md` |
| `semgrep-rules/prompt-build.yml` | a prompt assembled by concatenation from a variable | `surfaces/agents.md` |
| `corpora/prompt-injection.md` | the agent-surface payload classes | `surfaces/agents.md` |
| `scripts/fuzz-cli.py` | crash, hang, unparsable output, and guard-bypass invariants for any CLI or JSON-stdin program | every surface |

These work offline. `semgrep --config auto` reaches the network, so it needs the
user's agreement.

## Out of scope: LLM red-teaming

`garak` and `promptfoo` are excluded by design. Both measure whether a *model* can be
made to misbehave -- garak's own README says it does "somewhat similar things to nmap
or Metasploit, but for LLMs". The target here is this codebase, so a model's alignment
score answers a question nobody asked and costs inference to get.
