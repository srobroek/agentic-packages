...
| semgrep | default-on | local | `semgrep --config "$BREAK_SKILL_DIR/references/semgrep-rules/shell-injection.yml" --json <files>` | `eval` on interpolated data, `curl \| sh`, unsafe `mktemp`, missing `set -euo pipefail` | complements shellcheck with taint-shaped rules |
