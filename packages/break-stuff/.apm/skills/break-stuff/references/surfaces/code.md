...
|------|------|-------|-----------|---------|---------|
| semgrep | default-on | local | `semgrep --config auto --json <files>` plus `--config "$BREAK_SKILL_DIR/references/semgrep-rules/taint-sinks.yml"` | injection, unsafe deserialization, hardcoded secrets, weak crypto, path traversal | the cross-language floor; intra-file dataflow |
...
MUST Pass the shipped ruleset as an absolute path under `$BREAK_SKILL_DIR`, because cwd is the target repo and a relative path silently matches nothing.
