...
#   agentcfg snyk-agent-scan agentic-radar       (STATIC audit of MCP/agent CONFIG. NOT LLM
#                                                 red-teaming: garak and promptfoo attack a
#                                                 MODEL, which is a different target.)
...
BUNDLES="core shell python rust go js-ts native infra ci deps supply mutate triage schema grammar agentcfg deep"
...
tools_for() {
  ...
  case "$1" in
    core)
      ...
      echo "opengrep|opengrep|ubi|mise use ubi:opengrep/opengrep  (LGPL semgrep fork)|opengrep|ubi:opengrep/opengrep|mise x ubi:opengrep/opengrep -- opengrep"
    ...
    rust)
      ...
      echo "miri|cargo-miri|rustup|rustup +nightly component add miri  (interprets MIR to find real UB in unsafe)|miri||cargo +nightly miri"
      echo "cargo-deny|cargo-deny|cargo|mise use cargo:cargo-deny  (advisory, licence, banned-crate policy)|cargo-deny|cargo:cargo-deny|cargo deny"
    ...
    supply)
      ...
      echo "kingfisher|kingfisher|ubi|mise use ubi:mongodb/kingfisher  (secrets scanner that LIVE-VALIDATES a hit against the provider)|kingfisher|ubi:mongodb/kingfisher|mise x ubi:mongodb/kingfisher -- kingfisher"
    ...
    agentcfg)
      ...
      echo "semgrep-tob|semgrep|pipx|uvx semgrep --config p/trailofbits  (Trail of Bits rule pack; fetches over the network, so it needs agreement)|semgrep||uvx semgrep --config p/trailofbits"
      ;;
    deep)
      ...
      echo "joern|joern|ubi|mise use ubi:joernio/joern  (code property graph; real interprocedural taint)|joern|ubi:joernio/joern|mise x ubi:joernio/joern -- joern"
      echo "weggli|weggli|cargo|mise use cargo:weggli  (C/C++ semantic pattern search)|weggli|cargo:weggli|"
      echo "bearer|bearer|ubi|mise use ubi:Bearer/bearer  (dataflow SAST for sensitive-data leaks)|bearer|ubi:Bearer/bearer|mise x ubi:Bearer/bearer -- bearer"
...
usage: install-tools.sh [--probe | --list | --install <bundle>... | --all] [--dry-run]
  ...
  --install <bundle>   install the named bundle(s): core shell python rust go
                       js-ts native infra ci deps mutate triage schema grammar
                       deep
