...
def _load_sdk():
    ...
    Resolves $PLUGIN_ROOT (set by the executor) → runner/sdk.py, with a
    __file__-relative fallback for direct/dev invocation. Registers the loaded
    module in sys.modules BEFORE exec_module (mandatory — §6).
    """
    plugin_root = os.environ.get("PLUGIN_ROOT")
    if plugin_root:
        sdk_path = Path(plugin_root) / "runner" / "sdk.py"
        if not sdk_path.is_file():
            # PLUGIN_ROOT may point at the package root; try the skill subdir.
            sdk_path = Path(plugin_root) / "skills" / "project-setup" / "runner" / "sdk.py"
    else:
        # modules/<id>/module.py → ../../runner/sdk.py
