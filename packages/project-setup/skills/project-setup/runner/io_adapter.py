...
# --------------------------------------------------------------------------- #
# RecordingIO — wraps another IO and records all calls                        #
# --------------------------------------------------------------------------- #
class RecordingIO:
    """Wraps another ``InterviewIO`` and records every call for post-hoc inspection."""
    ...
    def ask(self, input_spec: dict[str, Any], default: Any) -> Any:
        result = self.delegate.ask(input_spec, default)
        self.log.append({"op": "ask", "input_spec": input_spec, "default": default, "result": result})
        return result

    def confirm(self, item: dict[str, Any]) -> bool:
        result = self.delegate.confirm(item)
        self.log.append({"op": "confirm", "item": item, "result": result})
        return result
    ...
    def agent_step(
        ...
        result = self.delegate.agent_step(steering_path, context)
        self.log.append({
            "op": "agent_step",
            "steering_path": steering_path,
            "context": context,
