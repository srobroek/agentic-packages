...
4. Consume JSON `pr-lifecycle` records as read-only state changes and `dispatch` records as agent-owned work slots. The runtime ranks ready PRs by priority label, enqueue time, repository, then PR number.
...
MUST Treat lifecycle and dispatch records as input only; the consuming shepherd owns Beads, gates, conflict probes, and merge mutations.
