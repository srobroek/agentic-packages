...
4. Consume JSON `pr-lifecycle` records as read-only state changes and `dispatch` records as agent-owned work slots. Resolve an exact orchestrate node first; only unmatched records may route once to pr-shepherd. The runtime ranks ready PRs by priority label, enqueue time, repository, then PR number.
