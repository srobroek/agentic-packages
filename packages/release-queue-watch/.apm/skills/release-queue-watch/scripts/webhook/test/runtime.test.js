...
test("writes verified and deduplicated pull request lifecycle records", async (t) => {
  const stateDir = await mkdtemp(join(tmpdir(), "release-queue-runtime-lifecycle-"));
  const output = [];
  const runtime = await startReleaseQueueRuntime(
    {
      repository: "owner/repo",
...
test("writes reconciled lifecycle records without changing dispatch records", async (t) => {
  const stateDir = await mkdtemp(join(tmpdir(), "release-queue-runtime-reconcile-"));
  const output = [];
  ...
  const lifecycle = output.filter((record) => record.type === "pr-lifecycle");
  assert.deepEqual(
    lifecycle.map((record) => [record.pullRequest.number, record.transition, record.source]),
    [
  ...
  assert.ok(output.indexOf(dispatch) < output.findIndex((record) => record.type === "watcher-active"));
});
