...
function splitRepository(repository) {
  ...
  if (!owner || !repo || extra) throw new Error(`invalid repository: ${repository}`);
  return { owner, repo };
}
...
export function combinedChecksState(checkRuns, combinedStatus) {
  ...
  const checksPass = checkRuns.every((run) => PASSING_CHECK_CONCLUSIONS.has(run.conclusion));
  ...
  async listOpenPullRequests(repository) {
    ...
    return Promise.all(
      pulls.map(async (pull) => {
        const [{ data: detail }, checkRuns, { data: combinedStatus }] = await Promise.all([
          this.octokit.rest.pulls.get({ owner, repo, pull_number: pull.number }),
          ...
          this.octokit.rest.repos.getCombinedStatusForRef({ owner, repo, ref: pull.head.sha }),
        ]);
        return {
          number: pull.number,
          title: pull.title,
