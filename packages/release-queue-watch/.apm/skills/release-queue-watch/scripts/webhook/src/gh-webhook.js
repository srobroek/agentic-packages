...
export async function ensureGhWebhookExtension({
  ...
  const { stdout } = await execFileFn("gh", ["extension", "list"], { env: childEnv });
...
export function buildForwardArgs({ repository, organization, events, secret, url }) {
...
export async function startGhWebhookForwarder({
  ...
  const args = buildForwardArgs({ repository, organization, events, secret, url });
...
export async function readGhToken({ env = process.env, execFileFn = execFile } = {}) {
