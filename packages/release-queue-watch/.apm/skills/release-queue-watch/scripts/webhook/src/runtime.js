...
import { startGhWebhookForwarder, readGhToken } from "./gh-webhook.js";
...
export function parseArgs(argv, env = process.env) {
  ...
  for (let index = 0; index < argv.length; ) {
    ...
    if (option.name === "--repo") options.repository = option.value;
      ...
      options.pollIntervalMs = integer(option.value, "poll-interval-ms", { minimum: 1_000 });
...
export async function startReleaseQueueRuntime(options, dependencies = {}) {
  ...
  const onDispatch =
    ...
    ((item) => logger.log(JSON.stringify({ type: "dispatch", pullRequest: item })));
  const reconciler = new PollingReconciler({
    ...
    onError: (error, repository) =>
      logger.error(JSON.stringify({ type: "reconcile-error", repository, message: error.message })),
  });
  const receiver = await createWebhookReceiver({
    ...
    onError: (error) =>
      logger.error(JSON.stringify({ type: "webhook-error", message: error.message })),
  });
