...
# then assert the deny/allow decision plus that the script PARSES under
...
# Run the guard; capture $output, $status, and decoded $decision (empty => allow).
run_guard() {
  payload="$1"
...
# --- direct edit of a managed file -> deny ---------------------------------

@test "managed file direct edit -> deny" {
  with_chezmoi
  run_guard "$(mk_edit "$MANAGED_FILE")"
  [ "$decision" = "deny" ]
}

# --- ../-traversal equivalent of a managed file -> deny (proves the fix) ----

@test "../-traversal equivalent of managed file -> deny (canonicalization)" {
  with_chezmoi
...
@test "../-traversal via parent of managed file -> deny" {
  with_chezmoi
...
# --- read-only command referencing a managed path -> allow ------------------
...
# --- shell write to a managed file -> deny ----------------------------------

@test "redirect overwrite of managed file -> deny" {
  with_chezmoi
  run_guard "$(mk_cmd "echo hi > $MANAGED_FILE")"
  [ "$decision" = "deny" ]
}

@test "tee into managed file -> deny" {
  with_chezmoi
  run_guard "$(mk_cmd "echo hi | tee $MANAGED_FILE")"
  [ "$decision" = "deny" ]
}

@test "cp destination is managed file -> deny" {
  with_chezmoi
  run_guard "$(mk_cmd "cp /tmp/src $MANAGED_FILE")"
  [ "$decision" = "deny" ]
}

@test "cp SOURCE is managed (dest unmanaged) -> allow" {
  with_chezmoi
...
# --- string-form tool_input write to managed file -> deny (proves the fix) --

@test "string-form tool_input edit of managed file -> deny (no bypass)" {
  with_chezmoi
...
@test "string-form tool_input redirect write to managed file -> deny (falls through to command check)" {
  with_chezmoi
  run_guard "$(mk_str "echo x > $MANAGED_FILE")"
  [ "$decision" = "deny" ]
}

# --- unmanaged file -> allow -----------------------------------------------
