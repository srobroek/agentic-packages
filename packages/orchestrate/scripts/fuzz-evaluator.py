...
def invoke(script, payload_str, env=None):
    ...
    runs += 1
    try:
        p = subprocess.run(["uv", "run", "--quiet", script], input=payload_str,
...
def must_be_valid_json_or_empty(script, out, rc, crashed, label):
    if crashed:
        ...
        return None
    if rc != 0:
        findings.append(f"[{label}] non-zero exit {rc} from {os.path.basename(script)}: {out[:120]}")
    s = out.strip()
    if s == "":
        findings.append(f"[{label}] EMPTY stdout (hooks must emit JSON) from {os.path.basename(script)}")
        return None
    try:
        return json.loads(s)
    except Exception:
...
def prop_fuzz():
    ...
    def claim_deny_never_crashes(cmd):
        out, rc, crashed = invoke(DENY, json.dumps({"tool_input": {"command": cmd}}),
...
def attack_vectors():
    ...
    v = ev(bead(id="a1", status="closed", labels=["agent:reviewer"],
    ...
    v = ev(bead(id="a2", status="in_progress", labels=["agent:reviewer"],
    ...
    v = ev(bead(id="a3", status="in_progress", labels=["agent:reviewer"],
                metadata={"execution_kind": "git", "branch": "b", "push": "s"},
                comments=[{"text": "i have not REPORTED anything yet"}]))
    if not is_block(v):
        findings.append("[attack] non-leading 'REPORTED' token satisfied completion (verb-parse bypass!)")

    # 4. Label injection: an evil label that merely contains agent: mid-string.
    v = ev(bead(id="a4", status="in_progress", labels=["not-an-agent:reviewer-really"],
                metadata={"execution_kind": "git", "branch": "b", "push": "s"},
                comments=[{"text": "REPORTED"}]))
    # ^agent: is anchored, so this label should NOT satisfy handoff -> block.
    if not is_block(v):
        findings.append("[attack] mid-string 'agent:' label satisfied ^agent: anchor (regex bypass!)")

    # 5. Unicode / emoji in comment verb position.
    v = ev(bead(id="a5", status="in_progress", labels=["agent:reviewer"],
                metadata={"execution_kind": "git", "branch": "b", "push": "s"},
                comments=[{"text": "🚀 REPORTED done"}]))
    ...
    big = bead(id="a6", status="in_progress", labels=["agent:reviewer"],
               metadata={"execution_kind": "git", "branch": "b", "push": "s"},
    ...
    v = ev(bead(id="a7", status="in_progress", labels=["agent:reviewer"],
    ...
    v = ev(bead(id="a9", status="blocked", metadata={"execution_kind": "git"},
    ...
    # 10. claim-deny: sneaky --claim with tabs/newlines around it.
    out, rc, crashed = invoke(DENY, json.dumps({"tool_input": {"command": "bd\tupdate x\t--claim"}}),
                             env={"ORCHESTRATE_RUN": "1"})
    v = must_be_valid_json_or_empty(DENY, out, rc, crashed, "attack-deny")
    # Tab-separated --claim: current regex uses \s so this SHOULD deny; if it allows, note it.
    if isinstance(v, dict) and v.get("decision") != "deny":
        findings.append("[attack] tab-separated 'bd update --claim' NOT denied (whitespace-anchor gap)")
