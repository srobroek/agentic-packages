...
Which carrier holds a choice -- work-bead comment, `decision` bead, message wisp, artifact -- and the
promotion rule that makes it authoritative, are generic beads doctrine and live in the beads package:
`beads.carriers.context.md`. Read it before writing any run state.

Orchestrate adds no carrier of its own. The rest of this file maps orchestrate's run model onto those
carriers.
