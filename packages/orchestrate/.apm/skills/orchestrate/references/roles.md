...
| **Builder** | `builder` (bundled) | **mid tier** medium | per node, kept alive across fix rounds | do **not** upgrade the builder — on a reasoning block it raises `BLOCKED` |
...
Workflow roles ship **bundled** with this package (builder, reviewer, advisor, shepherd, scribe); quality-guard roles (docs-guard, lint-guard, data-metrics-summarizer, maintenance-metrics-reader, reviewer-mechanics) come from the `agent-quality-guards` dependency; the remaining routes are built-in agents (`Explore`,
...
| Builder | its `scope` only | **nothing** | own git worktree | commits + pushes its branch; on block → `BLOCKED kind:design\|debug` to `main` |
...
4. Builder⇄reviewer deadlock after bounded fix rounds, or shepherd⇄builder conflict
