...
# Reclaim disk even when removal was refused (e.g. locked or unmanaged tree):
# build artifacts are reproducible, so a stranded worktree must not keep
# holding them. Only delete directories git itself marks as ignored, so no
# real work is ever touched.
if [ -d "$TOPLEVEL" ]; then
  for artifact_dir in target node_modules dist .venv; do
    if [ -d "$TOPLEVEL/$artifact_dir" ] && \
       git -C "$TOPLEVEL" check-ignore -q "$artifact_dir" 2>/dev/null; then
