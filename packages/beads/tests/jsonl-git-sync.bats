...
#   - beads-jsonl-export.sh  PreToolUse:Bash, refresh + stage on `git commit`
#   - beads-jsonl-import.sh  SessionStart, hydrate from the committed file
#
...
setup() {
  ...
  EXPORT="$S/beads-jsonl-export.sh"
  IMPORT="$S/beads-jsonl-import.sh"
...
run_export() { commit_payload "$1" | /bin/bash "$EXPORT"; }
run_import() { jq -n --arg cwd "$REPO" '{cwd:$cwd}' | /bin/bash "$IMPORT"; }
...
@test "scripts parse under /bin/bash" {
  run /bin/bash -n "$EXPORT"; [ "$status" -eq 0 ]
  run /bin/bash -n "$IMPORT"; [ "$status" -eq 0 ]
}

# --- fail-open gating -------------------------------------------------------

@test "export: empty payload exits 0 and writes nothing" {
  run bash -c "printf '' | /bin/bash '$EXPORT'"
  [ "$status" -eq 0 ]
  [ ! -f "$REPO/.beads/issues.jsonl" ]
}

@test "import: empty payload exits 0" {
  run bash -c "printf '' | /bin/bash '$IMPORT'"
  [ "$status" -eq 0 ]
}

@test "export: no beads workspace is inert" {
  run_export "git commit -m x"
  [ ! -f "$REPO/.beads/issues.jsonl" ]
}

@test "export: beads workspace WITHOUT opt-in is inert" {
  init_beads
...
@test "import: beads workspace WITHOUT opt-in is inert" {
  init_beads
  run run_import
  [ "$status" -eq 0 ]
  [ -z "$output" ]
...
@test "export: non-commit git command does not stage" {
  init_beads; opt_in
  bd -C "$REPO" create "a bead" >/dev/null 2>&1
  run_export "git status"
  [ -z "$(staged)" ]
}

@test "export: 'git commit' inside a quoted string does not stage" {
  init_beads; opt_in
  bd -C "$REPO" create "a bead" >/dev/null 2>&1
  # Quote-stripping guards this: an unrelated command that merely mentions a
  # commit must not rewrite and stage bead state.
  run_export 'echo "remember to git commit later"'
  [ -z "$(staged)" ]
}

@test "export: real commit stages the export even when the message says 'git commit'" {
  init_beads; opt_in
  bd -C "$REPO" create "a bead" >/dev/null 2>&1
  run_export 'git commit -m "docs: explain git commit"'
  [ "$(staged)" = ".beads/issues.jsonl" ]
}

@test "export: flags between git and commit still match" {
  init_beads; opt_in
  bd -C "$REPO" create "a bead" >/dev/null 2>&1
  run_export "git -c user.name=x commit -m y"
  [ "$(staged)" = ".beads/issues.jsonl" ]
}

# --- export: content --------------------------------------------------------

@test "export: written file is valid JSONL carrying the bead" {
  init_beads; opt_in
...
@test "export: a git-ignored target reports instead of failing silently" {
  init_beads; opt_in
  bd -C "$REPO" create "a bead" >/dev/null 2>&1
  ...
  output="$(run_export 'git commit -m x')"
  [ -z "$(staged)" ]
...
@test "import: a peer bead in the committed file lands in the database" {
  init_beads; opt_in
...
@test "import: routine hydration is silent" {
  init_beads; opt_in
  bd -C "$REPO" create "a bead" >/dev/null 2>&1
  run_export "git commit -m x"
...
@test "import: a stale committed file cannot revert newer local work" {
  init_beads; opt_in
